            ...
            >
                ...
                <div className="flex flex-col h-full px-16 pt-14 pb-10">
                    ...
                    <div className="mb-6 flex-shrink-0">
                        ...
                        {description && (
                            <p
                                className="text-[14px] font-normal leading-[1.7] max-w-[520px]"
                    ...
                    {/* Table — no cell borders, only horizontal rules between rows */}
                    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                        {/* Header row */}
                        <div
                            className="flex flex-shrink-0"
                            style={{ borderBottom: '1px solid color-mix(in srgb, var(--stroke, #E5E7EB) 15%, transparent)' }}
                        >
                            {/* Feature column header */}
                            <div
                                className="flex items-center px-4 py-3"
                                style={{ width: '28%', minWidth: '160px' }}
                            >
                                <span
                                    className="text-[11px] font-medium uppercase tracking-widest"
                                    style={{ color: 'var(--background-text, #101828)', opacity: 0.35, fontFamily: 'var(--heading-font-family, Inter)', letterSpacing: '0.1em' }}
                                >
                                    Feature
                                </span>
                            </div>
                        ...
                        {/* Body rows — only horizontal 1px rules between rows */}
                            ...
                            {rows?.map((row, rowIndex) => (
                                <div
                                    ...
                                    style={{
                                        ...
                                        borderBottom: rowIndex < (rows?.length ?? 0) - 1
                                            ? '1px solid var(--stroke, #E5E7EB)'
                                            : 'none',
                                        opacity: rowIndex < (rows?.length ?? 0) - 1 ? 1 : 1,
                                    }}
                                >
                                    {/* Feature name cell — no right border */}
                                    <div
                                        ...
                                        >
                                            <span
                                                ...
                                                style={{
                                                    ...
                                                    opacity: col.highlighted ? 0.9 : 0.75,
                                                }}
                                            >
                                                {row.values?.[colIndex] ?? '—'}
                                            </span>
