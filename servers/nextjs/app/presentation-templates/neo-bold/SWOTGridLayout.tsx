                ...
                <div className="flex flex-col h-full px-14 pt-10 pb-8">
                    ...
                    <div className="flex-1 grid grid-cols-2 grid-rows-2 gap-5 min-h-0">
                        {quadrants?.map((quadrant, index) => (
                            <div
                                key={index}
                                className="rounded-xl px-7 py-6 flex flex-col overflow-hidden shadow-xl"
                                style={{
                                    backgroundColor: 'var(--card-color, #F3F4F6)',
                                    borderTop: `6px solid ${quadrantAccents[index]?.border ?? 'var(--primary-color, #6366F1)'}`,
                                }}
                            >
                                {/* Quadrant label — 18px extrabold uppercase */}
                                <h3
                                    className="font-extrabold uppercase tracking-widest mb-3 flex-shrink-0"
                                    style={{
                                        fontSize: '18px',
                                        ...
                                        <li key={itemIndex} className="flex items-start gap-3">
                                            <span
                                                ...
                                                style={{
                                                    width: '7px',
                                                    height: '7px',
                                                    backgroundColor: quadrantAccents[index]?.dot ?? 'var(--primary-color, #6366F1)',
                                                }}
                                            />
                                            <span
