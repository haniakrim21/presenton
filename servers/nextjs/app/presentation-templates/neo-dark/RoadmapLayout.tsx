...
const RoadmapLayout: React.FC<{ data: Partial<z.infer<typeof Schema>> }> = ({ data }) => {
    ...
    const getNodeStyle = (status: 'completed' | 'in-progress' | 'upcoming') => {
        if (status === 'completed') {
            return {
        ...
        if (status === 'in-progress') {
            return {
                backgroundColor: 'var(--primary-color, #6366F1)',
                boxShadow: '0 0 16px var(--primary-color, #6366F1)',
        ...
        return {
            ...
            border: '3px solid var(--stroke, #E5E7EB)',
        };
    };
                ...
                <div className="flex flex-col h-full px-14 pt-12 pb-10">
                    ...
                    <div className="flex-1 flex flex-col justify-center min-h-0">
                        <div className="relative w-full" style={{ height: '320px' }}>
                            {/* Glowing horizontal timeline line */}
                            <div
                            ...
                            {milestones?.map((milestone, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        {/* Card above the timeline — inverted panel */}
                                        {isAbove && (
                                            <div
                                                className="mb-3 rounded-lg px-4 py-3 text-left"
                                                style={{
                                            ...
                                            >
                                                <p
                                                    ...
                                                    style={{ color: 'var(--primary-text, #ffffff)', opacity: 0.7, fontFamily: 'var(--heading-font-family, Inter)', letterSpacing: '0.07em' }}
                                                >
                                                    {milestone.phase}
                                                </p>
                                                <h3
                                        ...
                                        <div
                                            ...
                                            style={getNodeStyle(milestone.status)}
                                        />

                                        {/* Card below the timeline — inverted panel */}
                                        {!isAbove && (
                                            <div
                                                className="mt-3 rounded-lg px-4 py-3 text-left"
                                                style={{
                                                    width: `${Math.max(140, Math.floor(820 / count) - 16)}px`,
                                                    backgroundColor: 'var(--primary-color, #6366F1)',
                                                    boxShadow: '0 4px 20px rgba(0,0,0,0.25), inset 0 0 0 1px rgba(255,255,255,0.1)',
