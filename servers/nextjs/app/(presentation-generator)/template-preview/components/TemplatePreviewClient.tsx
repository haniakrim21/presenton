...
const GroupLayoutPreview = () => {
  ...
  useEffect(() => {
    const loadCustomLayouts = async () => {
      ...
      try {
        const res = await fetch(getApiUrl(`/api/v1/ppt/template-management/get-templates/${presentationId}`), {
          headers: getHeader(),
        });
        if (!res.ok) return;
  ...
  const deleteLayouts = async () => {
    refetch();
    router.back();
    const response = await fetch(getApiUrl(`/api/v1/ppt/template-management/delete-templates/${presentationId}`), {
      method: "DELETE",
      headers: getHeader(),
    });
    if (response.ok) {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(getApiUrl(`/api/v1/ppt/template-management/save-templates`), {
        method: "POST",
        headers: getHeader(),
